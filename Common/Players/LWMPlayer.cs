...
using LivingWorldMod.Content.Items.Tools;
using LivingWorldMod.Content.Projectiles.Friendly;
using LivingWorldMod.Custom.Classes.Packets;
...
namespace LivingWorldMod.Common.Players {
    ...
    public class LWMPlayer : ModPlayer, IBinarySerializable {
        public Guid guid;
        ...
        /// <summary>
        /// Array for all the possible types of vanilla wings to create the different visuals of the
        /// feather bag
        /// </summary>
        ...
        /// <summary>
        /// The amount of time (in ticks) between each jump that must pass before the feather burst
        /// can trigger again
        /// </summary>
        ...
        /// <summary>
        /// A cache of the item index as found during CanBuyItem. Used in PostBuyItem to update the
        /// item slot after buying, and put back the item but with an X over it, when the stack runs out.
        /// </summary>
        private int itemIdx;

        /// <summary>
        /// A cache for the GlobalItem of the item found during CanBuyItem. When the last item in a
        /// slot is purchased, the slot is reset, however we need to know if that was an original
        /// shop slot or not, and have a reference to the DailyShopData instance. So, we cache the
        /// LWMGlobalShopItem before buying to reference it after. We can't use the held item either
        /// because that gets reset after the first item in the held stack.
        /// </summary>
        ...
        private void RoseMirrorItem() {
            if (player.HeldItem.type == ModContent.ItemType<RoseMirror>() && player.itemAnimation > 0) {
                ...
                if (player.itemAnimation == player.HeldItem.useAnimation / 2) {
                    ...
                    if (LWMWorld.GetShrineTilePosition(VillagerID.Harpy) != Vector2.Zero) {
                        player.UnityTeleport(LWMWorld.GetShrineWorldPosition(VillagerID.Harpy) +
        ...
        public override void PostBuyItem(NPC vendor, Item[] shopInventory, Item heldItem) {
            ...
            if (heldGlobalItem.isOriginalShopSlot) {
                ...
                // not sure why I have to reduce the item value manually, but for some reason it
                // refuses to do it automatically, so here we are
                if (heldItem.value > 0) {
                    heldItem.value /= 5;
                    if (heldItem.value < 1)
                        heldItem.value = 1;
