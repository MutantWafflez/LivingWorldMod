...
namespace LivingWorldMod.NPCs.Hostiles
{
    public class SacSpiderWalled : ModNPC
    {
        ...
        public override void AI()
        {
			...
			Vector2 centerPlaceholder = new Vector2(npc.position.X + (float)npc.width * 0.5f, npc.position.Y + (float)npc.height * 0.5f);
			float npcXCenter = target.position.X + (float)(target.width / 2);
			float npcYCenter = target.position.Y + (float)(target.height / 2);
			...
			if (target.dead)
			{
				npcXCenter = (float)npc.direction * mathVariable / 2f;
