...
public class HarpyVillage : WorldGenFeature {
    ...
    public override void Generate(GenerationProgress progress, GameConfiguration gameConfig) {
        ...
        WorldUtils.Gen(
            originPoint,
            new Shapes.Slime(radius, xScale, yScale),
            Actions.Chain(
        ...
        WorldUtils.Gen(
            originPoint,
            new Shapes.Slime(radius, xScale * 0.75f, yScale * 0.75f),
            Actions.Chain(
                new Modifiers.Flip(false, true),
        ...
        WorldUtils.Gen(
            originPoint,
            new Shapes.Rectangle((int)(radius * xScale) * 2, 3),
            Actions.Chain(
        ...
        WorldUtils.Gen(
            originPoint,
            new ModShapes.All(mainIslandData),
            Actions.Chain(
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int xOffset = leftOffset * (1 - i); xOffset <= 0 + i * Math.Abs(leftOffset); xOffset++) {
                if (WorldUtils.Find(
                        new Point(originPoint.X + xOffset, originPoint.Y),
                        Searches.Chain(new Searches.Up(25), new Conditions.IsTile(TileID.Grass).AreaAnd(groundHouseData.structureWidth, 1)),
                        out Point groundHouseResult
                    )) {
        ...
        if (WorldUtils.Find(
                new Point(originPoint.X - churchBuildingData.structureWidth / 2, originPoint.Y + upOffset),
                Searches.Chain(new Searches.Up(75), new IsAir().AreaAnd(churchBuildingData.structureWidth, churchBuildingData.structureHeight)),
                out Point churchResult
            )) {
        ...
        for (int i = -1; i < 2; i += 2) {
            for (int j = 0; j < (currentWorldSizeY > WorldSize.Small ? 2 : 1); j++) {
                ...
                WorldUtils.Gen(
                    miniIslandOrigin,
                    new Shapes.Slime(miniRadius, 1f, miniYScale),
                    Actions.Chain(
                        new Modifiers.Flip(false, true),
                ...
                WorldUtils.Gen(
                    miniIslandOrigin,
                    new ModShapes.All(miniIslandData),
                    Actions.Chain(
